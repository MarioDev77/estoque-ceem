...
// ============================================================
router.post('/consumo/simular', requirePermission('consumo'), (req, res) => {
  const b = req.body || {};
  ...
  const ingredients = query('SELECT ri.*, f.name AS food_name, f.unit FROM recipe_ingredients ri JOIN foods f ON f.id = ri.food_id WHERE ri.recipe_id = ?', [recipe_id]);

  const items = ingredients.map((ing) => {
    const qty = calcTotalQuantity(ing.quantity_per_serving, ing.unit, people);
    const stock = get('SELECT quantity FROM stock WHERE food_id = ?', [ing.food_id]);
  ...
  const totalCost = items.reduce((acc, it) => {
    const food = get('SELECT avg_price FROM foods WHERE id = ?', [it.food_id]);
    return acc + it.quantity * (food ? food.avg_price : 0);
  }, 0);
...
router.post('/consumo/preview', requirePermission('consumo'), (req, res) => {
  ...
  const rows = items.map((it) => {
    ...
    const stock = get('SELECT quantity FROM stock WHERE food_id = ?', [food.id]);
    return {
...
// ============================================================
router.post('/consumo', requirePermission('consumo', 'can_create'), (req, res) => {
  const b = req.body || {};
  ...
  if (recipe_id) {
    const ingredients = query('SELECT ri.*, f.unit FROM recipe_ingredients ri JOIN foods f ON f.id = ri.food_id WHERE ri.recipe_id = ?', [recipe_id]);
  ...
  const preview = consumptionItems.map((ci) => {
    const qty = calcTotalQuantity(ci.portion, ci.unit, people);
    const stock = get('SELECT quantity FROM stock WHERE food_id = ?', [ci.food_id]);
    return { food_id: ci.food_id, qty, available: stock ? stock.quantity : 0, unit: ci.unit };
  });
  ...
  transaction(() => {
    ...
    if (menu_id) {
      run(`UPDATE menus SET status='realizado', expected_students=? WHERE id=?`, [people, menu_id]);
    }

    // cria refeição
    run(`INSERT INTO meals (menu_id, meal_type_id, date, planned_students, served_students, recipe_id, status, registered_by, notes)
         VALUES (?,?,?,?,?,?,'realizado',?,?)`,
      [menu_id ? num(menu_id) : null, mt, mealDate, people, people, recipe_id ? num(recipe_id) : null, req.user.id, str(notes, 500)]);
    const mealId = get('SELECT last_insert_rowid() AS id').id;
    ...
    for (const ci of consumptionItems) {
      ...
      // debita lotes FEFO
      let remaining = qty;
      const batches = query('SELECT * FROM food_batches WHERE food_id = ? AND quantity > 0 ORDER BY expiry_date ASC, id ASC', [ci.food_id]);
      ...
      // ajusta estoque
      run('UPDATE stock SET quantity = MAX(0, quantity - ?), updated_at = datetime(\'now\') WHERE food_id = ?', [qty, ci.food_id]);
      // registra consumo
      const planned = get('SELECT total_quantity FROM menu_items WHERE menu_id = ? AND food_id = ?', [menu_id, ci.food_id]);
      run(`INSERT INTO meal_consumption (meal_id, food_id, quantity, unit, planned_quantity)
           VALUES (?,?,?,?,?)`,
    ...
    if (leftovers) {
      run(`INSERT INTO leftovers (meal_id, date, meal_type_id, prepared_quantity, served_quantity, remaining_quantity, discarded_quantity, notes)
           VALUES (?,?,?,?,?,?,?,?)`,
  ...
  audit({ userId: req.user.id, action: 'registrar_consumo', module: 'consumo', entityType: 'meal', newValue: { date: mealDate, meal_type_id: mt, served_students: people, recipe_id } });
  notifyLowStock();
  res.json({ ok: true });
});

// Lista refeições realizadas
router.get('/consumo', requirePermission('consumo'), (req, res) => {
  ...
  const withDetails = rows.map((r) => ({
    ...r,
    consumption: query(`SELECT mc.*, f.name AS food_name, f.unit FROM meal_consumption mc JOIN foods f ON f.id = mc.food_id WHERE mc.meal_id = ?`, [r.id]),
  }));
...
// SOBRAS
// ============================================================
router.get('/sobras', requirePermission('sobras'), (req, res) => {
...
router.post('/sobras', requirePermission('sobras', 'can_create'), (req, res) => {
  const b = req.body || {};
  ...
  audit({ userId: req.user.id, action: 'registrar', module: 'sobras', entityType: 'leftover', newValue: b });
  res.json({ ok: true });
});
...
// ============================================================
router.get('/consumo/comparativo', requirePermission('consumo'), (req, res) => {
  const start = str(req.query.start, 10) || '1900-01-01';
  const end = str(req.query.end, 10) || '2100-12-31';
  res.json(query(`
