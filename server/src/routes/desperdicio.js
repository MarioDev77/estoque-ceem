...
router.get('/desperdicio', requirePermission('desperdicio'), async (req, res, next) => {
  try {
...
router.get('/desperdicio/indicadores', requirePermission('desperdicio'), async (req, res, next) => {
  try {
...
router.post('/desperdicio', requirePermission('desperdicio', 'can_create'), async (req, res, next) => {
  try {
    const b = req.body || {};
    ...
    await transaction(async (conn) => {
      await conn.query(`INSERT INTO waste (food_id, quantity, unit, reason, date, meal_id, estimated_cost, responsible, notes)
         VALUES (?,?,?,?,?,?,?,?,?)`,
      ...
      // debita estoque (FEFO)
      let remaining = qty;
      const batchesRes = await conn.query('SELECT * FROM food_batches WHERE food_id = ? AND quantity > 0 ORDER BY expiry_date ASC', [food_id]);
      ...
      await conn.query('UPDATE stock SET quantity = GREATEST(0, quantity - ?) WHERE food_id = ?', [qty, food_id]);
    });

    await audit({ userId: req.user.id, action: 'registrar', module: 'desperdicio', entityType: 'waste', newValue: { food_id, quantity: qty, reason } });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
