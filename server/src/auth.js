...
// Usa um hash aleatório derivado de entropia criptográfica (nunca uma string fixa).
...
export function isAccountLocked(email) {
  const row = get(
    ...
    [email, `-${LOGIN_LOCK_MINUTES} minutes`]
  );
...
export function recordLoginAttempt(email, ip, success) {
  try {
    get('INSERT INTO login_attempts (email, ip, success) VALUES (?,?,?)', [email, ip, success ? 1 : 0]);
  } catch (e) {
...
export function requireAuth(req, res, next) {
  ...
  const data = loadPermissions(payload.id);
  if (!data) {
